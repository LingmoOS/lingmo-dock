...
private:
    ApplicationItem* findItemByWId(const WindowId& wid);
    ...
    Q_SLOT void onWindowAdded(WindowId wid);
    Q_SLOT void onWindowRemoved(WindowId wid);
    Q_SLOT void onActiveChanged(WindowId wid);

private:
    std::shared_ptr<XWindowInterface> m_iface;
